...
import type { IndoorMapOptions, MapGL, MapGLWithIndoor } from './Types';
...
class MapServerHandler {
    ...
    private loadMapsIfNecessary = async (): Promise<void> => {
        ...
        if (this.downloadedBounds !== null) {
            if (
                bboxContains(this.downloadedBounds, viewPort.getNorthEast().toArray()) &&
        ...
        const southWest = turfDestination(center.toArray(), dist, (-3 * Math.PI) / 4).geometry
            .coordinates;
    ...
    private loadMapsInBounds = async (bounds: BBox): Promise<void> => {
        const url =
            this.serverUrl + `/maps-in-bounds/${bounds[0]},${bounds[1]},${bounds[2]},${bounds[3]}`;
    ...
    private addCustomMap = async (map: RemoteMap): Promise<void> => {
    ...
    private removeCustomMap = async (map: RemoteMap): Promise<void> => {
